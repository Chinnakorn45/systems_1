...
// หาแผนกหลักจากแผนกย่อย
$main_department = null;
if (!empty($department)) {
    $sql = "SELECT parent_id FROM departments WHERE department_name = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "s", $department);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $parent_id);
        if (mysqli_stmt_fetch($stmt)) {
            $main_department = $parent_id;
        }
        mysqli_stmt_close($stmt);
    }
}
...
<head>
    ...
    <title>แก้ไขโปรไฟล์ - ระบบบันทึกคลังครุภัณฑ์</title>
    ...
    <style>
        ...
        /* ซ่อนปุ่มตา/ล้างค่าเริ่มต้นที่เบราว์เซอร์บางตัวใส่มา เพื่อลดการซ้อน */
        input[type="password"]::-ms-reveal,
        input[type="password"]::-ms-clear {
            display: none;
        }
        /* สำหรับเอนจิน WebKit บางเวอร์ชัน/ปุ่ม AutoFill */
            ...
            <div class="main-content mt-4 mt-md-5">
                <div class="container" style="max-width: 700px;">
                    ...
                    <form action="profile.php<?php echo $is_admin_edit ? '?id=' . $user_id : ''; ?>" method="post" class="mb-3">
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <select class="form-select" id="main_department" name="main_department" required>
                                    ...
                                    <?php foreach ($main_departments as $dep): ?>
                                        <option value="<?php echo $dep['department_id']; ?>" <?php if ($main_department == $dep['department_id']) echo 'selected'; ?>>
                        ...
                        <!-- ช่องรหัสผ่าน (เพิ่มปุ่มตาแบบ input-group) -->
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-outline-secondary toggle-password" type="button" data-target="password" aria-label="แสดง/ซ่อนรหัสผ่าน">
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-outline-secondary toggle-password" type="button" data-target="confirm_password" aria-label="แสดง/ซ่อนรหัสผ่าน">
                                        <i class="fa-regular fa-eye"></i>
                                    </button>
                        ...
                        <a href="users.php" class="btn btn-link mt-3">ยกเลิก</a>
                    </form>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    /* ====== ใหม่: ปุ่มรูปตา ดู/ซ่อนรหัสผ่าน (กันไอคอนซ้อน) ====== */
