...
$sql = "SELECT 
            ...
            WHEN EXISTS (SELECT 1 FROM borrowings b4 WHERE b4.item_id = i.item_id AND b4.status IN ('borrowed','pending','overdue')) THEN 'กำลังยืม'
            WHEN EXISTS (SELECT 1 FROM repairs r3 WHERE r3.item_id = i.item_id AND r3.status NOT IN ('completed','cancelled')) THEN 'ส่งซ่อม'
            WHEN EXISTS (SELECT 1 FROM equipment_movements em3 WHERE em3.item_id = i.item_id AND em3.movement_type IN ('maintenance','disposal')) THEN 'บำรุงรักษา'
            ELSE 'พร้อมใช้งาน'
            END AS status_text,
            (SELECT image_path 
                FROM item_images ii 
...
if (!function_exists('thaidate')) {
    ...
    $replaced = str_replace(date('Y', $ts), $thaiYear, $replaced);
    return $replaced;
    }
}
...
<div class="print-header">
    ...
    if ($status !== '')        $filters[] = "สถานะ: " . htmlspecialchars($status);
...
<script>
  ...
  // หลังจากปิดหน้าต่างพิมพ์ (ทั้งพิมพ์จริงหรือกดยกเลิก)
    ...
    // กลับไปยังหน้า items.php พร้อมพารามิเตอร์ตัวกรอง
    window.location.href = "items.php?<?= htmlspecialchars(http_build_query($_GET)) ?>";
    };
</script>
